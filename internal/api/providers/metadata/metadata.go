...
func (e *Enricher) buildSeasonMapFromSuperFlix(ctx context.Context, animeName string) []SeasonMapping {
	...
	// Step 1: Search SuperFlix. Use the canonical .best host directly: the
	// legacy .online host 301-redirects here and Go's http.Client downgrades
	// POSTs to GETs across the redirect, which breaks the player API.
	searchURL := "https://superflixapi.best/pesquisar?s=" + url.QueryEscape(cleanName)
	...
	epURL := "https://superflixapi.best/serie/" + tmdbID
	...
	req2.Header.Set("Referer", "https://superflixapi.best/")
